 ...
 * Bug #1:
 * 
 * Problem: Using setTimeout in loadTasks()
 * This defers loading tasks, but renderTasks() and updateTaskCount() run immediately 
 * in initApp() before tasks are actually set — leading to the “invisible tasks” bug.
 * 
 * Solution: Changing loadTasks() to synchronously assign tasks so they are available immediately
 * 
 */
 ...
 * Bug #3:
 * 
 * Problem: Same root issue as Bug #1 — tasks isn’t populated in time due to the deferred setTimeout(). 
 * So if the render happens before that callback fires, tasks won't appear.
 * 
 * Solution: Already fixed by the same synchronous update in Bug #1.
 * 
 */
 ...
 * Bug #2:
 * 
 * Problem: Correctly update currentFilter in setFilter(), but never re-render the task 
 * list afterward because renderTasks() is commented out
 * 
 * Solution: Uncommenting the line to refresh the visible task list when a filter is clicked.
 * 
 */
